...
We need an `UnsafeMutablePointer<Void>`. We can use the `&` operator to pass an address. The pointer must be mutable so we declare it as a `var`. The `Void` is there to specify that the type system does not know what type of data we are going to get out of the data, but we know that it is going to be a 16 bit integer. We need to initialize the `seq` variable to zero so the compiler can reserver and zero out some memory for us.
*/
        var seq: UInt16 = 0
